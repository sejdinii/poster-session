...
from pathlib import Path #so the csv path works no matter where the script is run from

load_dotenv() #loading our local .env file

#anchor the output to this scripts own location instead of wherever the
#terminal happens to be standing. the csv lands in a Dataset folder that sits
#right next to this script, and the folder is created automatically if its
#missing. want it somewhere else? change only this CSV_DIR line
BASE_DIR = Path(__file__).resolve().parent          #...\Neurips Dataset
CSV_DIR = BASE_DIR / "Dataset"                      #...\Neurips Dataset\Dataset
...
with open(CSV_DIR / "neurips_accepted_submissions.csv", 'w', newline='', encoding='utf-8') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Number', 'Title', 'Abstract', 'Authors', 'Primary Area'])  # header row
    ...
    for submission in submissions:
        ...
        #primary_area = the official neurips topic of the paper, same .get pattern as title and abstract
        primary_area = submission.content.get('primary_area', {}).get('value')
        ...
        writer.writerow([submission.number, title, abstract, "; ".join(authors), primary_area])